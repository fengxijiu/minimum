...
class FakeClient {
  private turns: any[][];
  constructor(turns: any[][]) { this.turns = [...turns]; }
...
describe('P0-2 hooks', () => {
  it('fires the full lifecycle', async () => {
    ...
    for await (const _ of loop.run('go')) { /* drain */ }
  ...
  it('PreToolUse non-zero exit blocks the tool', async () => {
    ...
    const hookManager = {
      async execute(event: string) {
        if (event === 'PreToolUse') return [{ success: false, stderr: 'denied by hook' }];
        return [];
      },
    };
    const tools = new ToolRegistry();
...
describe('P0-1 EngineBridge', () => {
  ...
  it('streams normalized events from a loop', async () => {
    ...
    expect(ui.some(e => e.kind === 'assistant' && e.text === 'hello')).toBe(true);
