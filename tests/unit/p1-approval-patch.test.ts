...
describe('ApprovalManager — three-tier modes', () => {
  const mkReq = (tool: string, args: Record<string, any> = {}) =>
    ({ id: 't1', tool, args, risk: 'medium' as const, description: '', timestamp: 0 });

  it('read-only blocks writes, allows reads', async () => {
    const mgr = new ApprovalManager({ mode: 'read-only' });
    const writeReq = await mgr.requestApproval('write_file', { path: 'a.ts' }, '');
    const readReq  = await mgr.requestApproval('read_file',  { path: 'a.ts' }, '');
  ...
  it('auto-edit allows file edits, blocks shell', async () => {
    ...
    const shellReq = await mgr.requestApproval('exec_shell',  { command: 'ls' }, '');
  ...
  it('full-auto approves everything including shell', async () => {
    ...
    const shellReq = await mgr.requestApproval('exec_shell', { command: 'rm -rf /' }, '');
    expect((await mgr.checkApproval(shellReq)).approved).toBe(true);
  });

  it('suggest auto-approves low-risk tools', async () => {
    const mgr = new ApprovalManager({ mode: 'suggest', autoApproveLowRisk: true });
...
describe('ApplyPatchTool', () => {
  ...
  it('applies a single hunk correctly', async () => {
    ...
    const out = await tool.execute(
      { path: 'a.ts', hunks: [{ search: 'const x = 1;', replace: 'const x = 99;' }] },
  ...
  it('applies multiple hunks in order', async () => {
    write('b.ts', 'a\nb\nc\n');
    await tool.execute(
      { path: 'b.ts', hunks: [{ search: 'a', replace: 'A' }, { search: 'c', replace: 'C' }] },
      { workingDirectory: dir },
    );
