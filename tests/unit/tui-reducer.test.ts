...
describe('TUI reducer', () => {
  ...
  it('assistant.final with explicit text uses that text', () => {
    ...
    expect(s.messages[0]).toMatchObject({ type: 'assistant', text: 'explicit' });
  });

  it('tool.start + tool.end updates tool status', () => {
    let s = reduce(base, { type: 'tool.start', id: 't1', name: 'read_file', args: 'foo.ts' });
    expect(s.messages).toHaveLength(1);
    expect(s.messages[0]).toMatchObject({ type: 'tool' });
    s = reduce(s, { type: 'tool.end', id: 't1', ok: true, meta: '48 ln' });
    expect(s.messages[0]).toMatchObject({ type: 'tool', tool: { status: 'ok', meta: '48 ln' } });
  });
  ...
  it('plan.step.update advances step', () => {
    ...
    const s = reduce(withPlan, { type: 'plan.step.update', index: 0, status: 'done' });
    expect(s.plan.steps[0]!.status).toBe('done');
  });

  it('edit.add appends to edits', () => {
    const s = reduce(base, { type: 'edit.add', edit: { sign: '+', label: 'foo.ts · bar' } });
    expect(s.edits).toHaveLength(1);
  });

  it('edit.remove removes by index', () => {
    const withEdits = reduce(base, { type: 'edit.add', edit: { sign: '+', label: 'a' } });
  ...
  it('error.push appends error message', () => {
    const s = reduce(base, { type: 'error.push', title: 'ERR', lines: ['line 1'] });
    expect(s.messages).toHaveLength(1);
    expect(s.messages[0]).toMatchObject({ type: 'error', error: { title: 'ERR', lines: ['line 1'] } });
  });
  ...
  it('tool.start sets activeTool', () => {
    const s = reduce(base, { type: 'tool.start', id: 't1', name: 'read_file', args: 'foo.ts' });
  ...
  it('tool.end with ok=false sets err status', () => {
    let s = reduce(base, { type: 'tool.start', id: 't1', name: 'exec_shell', args: 'pytest' });
  ...
  it('streaming chunks accumulate then finalize', () => {
    ...
    expect(s.streaming).toBeNull();
    expect(s.messages).toHaveLength(1);
    expect(s.messages[0]).toMatchObject({ type: 'assistant', text: 'Hello world' });
  });
  ...
  it('usage.update tracks cost', () => {
    const s = reduce(base, { type: 'usage.update', promptTokens: 100, completionTokens: 50, cost: 0.005 });
  ...
  it('edit.undo removes last edit and shows toast', () => {
    const withEdits = reduce(base, { type: 'edit.add', edit: { sign: '+', label: 'foo.ts' } });
