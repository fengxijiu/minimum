 ...
 * 创建模拟验证请求
 */
export function createMockValidationRequest(
	toolName = "test_tool",
): ValidationRequest {
	return {
		toolName,
 ...
 * 创建模拟完整性请求
 */
export function createMockCompletenessRequest(
	task = "test task",
): CompletenessRequest {
	return {
		task,
 ...
 * 创建模拟完整性结果
 */
export function createMockCompletenessResult(
	complete = true,
): CompletenessResult {
	return {
		complete,
