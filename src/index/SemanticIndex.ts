...
export class SemanticIndex {
	...
	constructor(options?: {
		...
		this.basePath =
			options?.basePath ||
			path.join(process.env.HOME || "~", ".minimum", "index");
		this.embeddingProvider =
			options?.embeddingProvider || new LocalEmbeddingProvider();
		this.config = {
	...
	async addDirectory(dirPath: string, recursive = true): Promise<number> {
		let count = 0;
	...
	private extractSnippet(
		...
		const end = Math.min(
			chunk.length,
			queryIndex + query.length + contextLength / 2,
		);
