...
5. **Refine** the DAG after the W1 perception stage using vision/scout/context
   reports, canonical memory, and embedded Context Builder guidance.
6. **Finalize** in W4: decide patch merge order and memory governance
...
- Any task that edits a manifest or lockfile (`package.json`, `*lock*`,
  ...
  in the same scheduler batch (avoids lockfile conflicts).
...
1. **Static unroll (this prompt, W0).** When you can bound the number of fix
  ...
  they never share a scheduler batch. Do not unroll more than one fix pass speculatively;
...
Never create more than 6 parallel tasks of the same persona in one scheduler batch unless
...
Tasks with `needs_refine: true` get final `allowedGlobs` after the W1 scan stage.
...
After the W1 perception stage, supply concrete paths for every `needs_refine` task
...
In W4 you receive: task reports, memory candidates, and current canonical
...
After finalize, you are the primary user-facing delivery agent for W4.
