...
function resolveGrammarPath(grammar: GrammarName, overrideDir?: string): string {
	...
	candidates.push(
		resolve(dirname(fileURLToPath(import.meta.url)), "..", "..", "..", "dist", "grammars", filename),
	);
	candidates.push(
	...
	for (const pkg of DEV_PACKAGE_FOR_GRAMMAR[grammar]) {
		try {
			candidates.push(
				resolve(dirname(localRequire.resolve(`${pkg}/package.json`)), filename),
			);
		} catch {
	...
	throw new Error(
		`tree-sitter grammar ${grammar} not found. Looked in: ${candidates.join(", ")}`,
	);
}
