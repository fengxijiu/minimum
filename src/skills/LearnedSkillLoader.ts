...
export interface LoadedLearnedSkill {
	name: string;
	description: string;
	tags: string[];
	triggers: string[];
...
export async function loadLearnedSkills(projectRoot: string): Promise<LoadedLearnedSkill[]> {
	const base = path.join(projectRoot, ".minimum", "skills", "learned");
	try {
		const entries = await fs.readdir(base, { withFileTypes: true });
		const skills: LoadedLearnedSkill[] = [];
...
export function loadLearnedSkillsSync(projectRoot: string): LoadedLearnedSkill[] {
	const base = path.join(projectRoot, ".minimum", "skills", "learned");
	try {
