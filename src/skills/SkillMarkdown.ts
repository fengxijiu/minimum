...
function parseSimpleYaml(yaml: string): Record<string, unknown> {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (line.startsWith("- ")) {
			if (Array.isArray(current)) current.push(parseScalar(line.slice(2)));
			continue;
		}

		const split = line.indexOf(":");
		if (split < 0 || Array.isArray(current)) continue;
...
function nextNonEmpty(lines: string[], start: number): string | undefined {
...
function parseScalar(value: string): unknown {
	...
	if (Number.isFinite(num) && /^-?\d+(?:\.\d+)?$/.test(unquoted)) return num;
	return unquoted;
}
