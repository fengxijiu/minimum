...
/**
 * MiMoPipeline — drive a user request through the W0–W4 orchestration pipeline.
 *
...
export interface PlannerBridge {
	...
	/** W2-plan: audit a worker-authored execution plan before write-capable execution. */
...
export interface PipelineOptions {
	...
	/** Optional cancellation signal forwarded by PipelineBridge. */
	signal?: AbortSignal;
...
function adaptHarnessEventToTaskResult(
	event: HarnessEvent,
	contractsById: ReadonlyMap<string, TaskContract>,
): TaskResult | undefined {
	if (event.type === "task_done" || event.type === "task_blocked" || event.type === "task_failed") {
		return event.result;
	}

	if (event.type === "task_skipped") {
		return createSkippedTaskResult(event.taskId, event.reason, contractsById);
	}

	return undefined;
}

function createSkippedTaskResult(
	taskId: string,
	reason: string,
	contractsById: ReadonlyMap<string, TaskContract>,
): TaskResult {
