    ...
    <Box flexDirection="column" paddingX={1} paddingTop={1} paddingBottom={2}>
      ...
      <Box marginTop={1} flexDirection="row" columnGap={4}>
        <Box flexDirection="column">
          ...
          <Text color={theme.inkSoft}>· <Text color={theme.accent}>@</Text> brings a file into context</Text>
          <Text color={theme.inkSoft}>· <Text color={theme.accent}>/</Text> for commands  ·  <Text color={theme.accent}>?</Text> for help</Text>
          <Text color={theme.inkSoft}>· <Text color={theme.accent}>tab</Text> flips <Text color={theme.muted}>chat ↔ agent</Text></Text>
        </Box>

        <Box flexDirection="column">
          <Text color={theme.muted}>TRY ASKING</Text>
          {SUGGESTED.map((s, i) => (
            <Box key={i}>
              <Text color={theme.line}>› </Text>
              <Text color={theme.inkSoft}>{s}</Text>
            </Box>
          ))}
        </Box>
      </Box>
